...
def test_one_param_set(
    ...
    train_acc = sum(y_pred_train == data_dict['y_train'])/y_pred_train.shape[0]
    y_pred_val = cls.predict(data_dict['X_val'])
    val_acc = sum(y_pred_val == data_dict['y_val'])/y_pred_val.shape[0]
    # Replace "pass" statement with your code
    pass
    ############################################################################
    #                            END OF YOUR CODE                              #
...
def softmax_loss_naive(
    ...
    C = W.size(dim=1)
    for i in range(N):
      mid_res = torch.matmul(X[i],W)
      mid_res -= torch.max(mid_res)
      mini_sum = torch.sum(torch.exp(mid_res))
      mini_sum_log = torch.log(mini_sum)
      loss += -mid_res[y[i]] + mini_sum_log
      for j in range(C):
        p = torch.exp(mid_res[j])/mini_sum
        if(y[i]==j):
          p-=1
        dW[:,j]+= p * X[i]
    loss /= N
...
def softmax_loss_vectorized(
    ...
    N = X.size(dim=0)
    C = W.size(dim=1)
    mid_res = torch.matmul(X,W)
    idx0 = torch.arange(mid_res.shape[0])
    mid_res[idx0,:] -= torch.max(mid_res[idx0])
     
    label_list = mid_res[idx0,y]
    #loss -= torch.sum(label_list)
    mini_sum = torch.sum(torch.exp(mid_res),dim=1)
    mini_sum_log = torch.log(mini_sum)
    loss += torch.sum(mini_sum_log)-torch.sum(label_list)
    mini_sum = torch.unsqueeze(mini_sum,1)
    mini_sum = mini_sum.repeat(1,C)
    # print(mid_res.shape)
    # print(mini_sum.shape)
    p = torch.exp(mid_res)/mini_sum
    p_idx = torch.arange(p.shape[0]) 
    p[p_idx,y] -=1
        
    dW+= torch.matmul (X.transpose(0,1) , p)
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-2,7*1e-3,6*1e-3,4*1e-3]
    regularization_strengths = [0.05,0.1,0.2,0.3]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
