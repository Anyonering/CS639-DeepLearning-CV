...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      inst_train = torch.flatten(x_train[i])
      for j in range(num_test):
        inst_test = torch.flatten(x_test[j])
        # length = inst_test.size()
        inst_diff = torch.square(inst_train-inst_test)
        dists[i,j] =  torch.sum(inst_diff)
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      inst_train = torch.flatten(x_train[i])
      inst_diff = torch.square(inst_train-torch.flatten(x_test,start_dim=1))
      dists[i] =  torch.sum(inst_diff, dim=1)
    dists = torch.sqrt(dists)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    flat_x_test = torch.flatten(x_test,start_dim=1)
    flat_x_train = torch.flatten(x_train,start_dim=1)
    sum_test = torch.sum(torch.square(flat_x_test),dim=1)
    sum_train = torch.sum(torch.square(flat_x_train),dim=1)
    dists +=sum_test.reshape([1,num_test])
    dists +=sum_train.reshape([num_train,1])
    dists +=  -2 * torch.matmul(flat_x_train, flat_x_test.transpose(0,1))
    dists = torch.sqrt(dists)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
      list_train = dists[:,i].tolist()
      train_diff = torch.topk(dists[:,i],k,largest=False)
      index_list = list(train_diff.indices)
      act_list = []
      for j in range(k):
        act_list.append(y_train[index_list[j]])
      #act_list = [y_train[index_list[j]] for j in index_list]
      current_max = act_list[0]
      current_max_count = 1
      for j in range(k):
        if(act_list.count(act_list[j])>current_max_count):
          current_max=act_list[j]
          current_max_count = act_list.count(act_list[j])
      y_pred[i] = current_max
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for i in range(len(k_choices)):
      sample_k = k_choices[i]
      k_to_accuracies[sample_k] = []
      for j in range(num_folds):
        x_train_set = tuple(x_train_folds[0:j])+tuple(x_train_folds[j+1:])
        y_train_set = tuple(y_train_folds[0:j])+tuple(y_train_folds[j+1:])
        #print(len(x_train_set))
        inst_x_train = torch.cat(x_train_set,0)
        inst_y_train = torch.cat(y_train_set,0)
        # inst_x_train = torch.cat(x_train_folds[0:j],0)
        # inst_x_train = torch.cat((inst_x_train,x_train_folds[j+1:]),0)
        
        dists = compute_distances_no_loops(inst_x_train,x_train_folds[j])
        y_test_pred = predict_labels(dists,inst_y_train,sample_k)
        num_samples = x_train_folds[j].shape[0]
        num_correct = (y_train_folds[j] == y_test_pred).sum().item()
        accuracy = 100.0 * num_correct / num_samples
        k_to_accuracies[sample_k].append(accuracy) 
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    # Replace "pass" statement with your code
    current_key_acc = 0
    for key_k in k_to_accuracies:
      sum = 0
      for i in range(len(k_to_accuracies[key_k])):
        sum +=k_to_accuracies[key_k][i]
      acc = sum/len(k_to_accuracies[key_k])
      if(acc>current_key_acc):
        current_key_acc = acc
        best_k = key_k
