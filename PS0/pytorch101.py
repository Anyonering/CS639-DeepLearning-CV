...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    tensor_size = ((stop - (stop%10)) - pivot)//10 + 1
    x = torch.empty((tensor_size), dtype=torch.float64)
    for i in range(tensor_size):
        x[i] = float(pivot + i * 10)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def slice_indexing_practice(x: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
    ...
    even_rows_odd_cols = x[0::2,1::2]#.clone()
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    out = (
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    size_x = len(x)
    size_c = max(x) + 1
    y = torch.zeros((size_x,size_c),dtype=torch.float32)
    idx1 = list(range(size_x))
    y[idx1,x] = 1
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y = x.reshape(2,3,4).contiguous().transpose(0,1).reshape(3,8)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    mu = (x.sum(dim = 0))/M
    y = y-mu
    devt = torch.sqrt(torch.sum(torch.mul(y,y),dim=0)/(M-1))
    y = y/devt
