...
def discriminator_loss(logits_real, logits_fake):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    loss_real = torch.nn.functional.binary_cross_entropy_with_logits(logits_real, torch.ones_like(logits_real))
    loss_fake = torch.nn.functional.binary_cross_entropy_with_logits(logits_fake, torch.zeros_like(logits_fake))
...
def run_a_gan(D, G, D_solver, G_solver, loader_train, discriminator_loss, generator_loss, device, show_images, plt, show_every=250, 
  ...
  for epoch in range(num_epochs):
    for x, _ in loader_train:
      ...
      d_total_error = None
      D_solver.zero_grad()
      noise_feat = sample_noise(batch_size=batch_size,noise_dim=noise_size,device=device)
      fake_images = G(noise_feat)
      real_images = ((x.to(device).view(batch_size,-1))-0.5)*2
      # print(fake_images.device)
      # print(real_images.device)
      d_total_error = discriminator_loss(D(real_images), D(fake_images))
