...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        out = torch.zeros(N,F,Hout,Wout, device=x.device,dtype=x.dtype)
        p2d = (pad, pad, pad, pad)
        pad_input = torch.nn.functional.pad(x,p2d,"constant", 0)
        for i in range(N):
            for j in range(F):
    ...
    def backward(dout, cache):
        ...
        N, C, H, W = x.shape
        F, _C, HH, WW = w.shape
        _N, _F, Hout, Wout = dout.shape
        pad_input = torch.nn.functional.pad(x,p2d,"constant", 0)

        dx = torch.zeros_like(pad_input, device=x.device, dtype=x.dtype)
